...
from webapp.config import COUNT_CARS
...
def create_app():
    ...
    def count_cars():
        return render_template("submit.html", main_text=COUNT_CARS)

    @app.route("/cars/", methods=["POST"])
    def process_count_cars():
        file_name = process_file(request)
        if file_name is None:
            log_and_flash("Ошибка загрузки файла")
